...
let squadMarkers = {}; 
let squadTraces = {}; 
let panicModeActif = false; // 🛠️ FIX : Déclaré tout en haut pour éviter le bug d'initialisation !
...
// --- 🕵️ IDENTIFICATION À L'OUVERTURE ---
document.addEventListener('DOMContentLoaded', () => {
    if (!callsign) {
        showSysModal('prompt', '🕵️ IDENTIFICATION', 'Bienvenue sur le réseau.\nEntrez votre Identifiant (Pseudo) :', '#3498db', (val) => {
            if(val && val.trim() !== '') { callsign = val.trim(); } else { callsign = "Agent X"; }
            localStorage.setItem('urbex_callsign', callsign); 
            document.getElementById('status-text').innerText = `👋 ${callsign}`;
            afficherNotification(`Connexion établie, ${callsign}.`, '#2ecc71');
        });
    } else {
        document.getElementById('status-text').innerText = `👋 ${callsign}`;
    }
...
// --- 💀 PROTOCOLE HOMME À TERRE (PERSISTANT) ---
let deadmanInterval;
let deadmanTimeRemaining = 0;

function setupDeadManSwitch() {
    hideAllPanels();
    showSysModal('prompt', '💀 PROTOCOLE HOMME À TERRE', 'Durée avant S.O.S (en minutes) :', '#8e44ad', (mins) => {
        if (mins && !isNaN(mins) && parseInt(mins) > 0) {
            const endTime = Date.now() + (parseInt(mins) * 60000);
            localStorage.setItem('urbex_deadman_end', endTime);
            resumeDeadManSwitch();
            afficherNotification(`💀 Protocole armé !`, "#8e44ad");
        }
    });
}

function resumeDeadManSwitch() {
    const endTime = localStorage.getItem('urbex_deadman_end');
    if (endTime) {
        deadmanTimeRemaining = Math.floor((parseInt(endTime) - Date.now()) / 1000);
        const timerEl = document.getElementById('deadman-timer');
        if (deadmanTimeRemaining > 0) {
            timerEl.classList.remove('hidden');
            timerEl.style.background = "#8e44ad";
            clearInterval(deadmanInterval);
            deadmanInterval = setInterval(updateDeadMan, 1000);
            updateDeadMan();
        } else {
            localStorage.removeItem('urbex_deadman_end');
            triggerSOSAuto();
        }
    }
}
...
function updateDeadMan() {
    ...
    const m = Math.floor(deadmanTimeRemaining / 60).toString().padStart(2, '0');
    const s = (deadmanTimeRemaining % 60).toString().padStart(2, '0');
    timerEl.innerText = `💀 ${m}:${s}`;
    if (deadmanTimeRemaining === 60) {
        timerEl.style.background = "#e74c3c";
        afficherNotification("⚠️ HOMME À TERRE : Plus qu'1 min !", "#e74c3c");
        if ("vibrate" in navigator) navigator.vibrate([500, 200, 500, 200, 500]);
    }
}

function triggerSOSAuto() {
    afficherNotification("🚨 HOMME À TERRE ! S.O.S AUTO !", "#c0392b");
    if (userGPSMarker) {
        socket.emit('sos_alert', { lat: userGPSMarker.getLatLng().lat, lng: userGPSMarker.getLatLng().lng }); 
    } else if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            pos => { socket.emit('sos_alert', { lat: pos.coords.latitude, lng: pos.coords.longitude }); },
            err => {}, { enableHighAccuracy: false, timeout: 10000, maximumAge: 10000 }
        );
    }
}

function cancelDeadManSwitch() {
    clearInterval(deadmanInterval);
    document.getElementById('deadman-timer').classList.add('hidden');
    localStorage.removeItem('urbex_deadman_end');
    afficherNotification("🔒 Protocole Homme à Terre désactivé.", "#95a5a6");
}

// --- ⏱️ CHRONO D'EXTRACTION (PERSISTANT) ---
let timerInterval; 
let timeRemaining = 0; 

function startTimerPrompt() { 
    hideAllPanels(); 
    showSysModal('prompt', '⏱️ CHRONO EXTRACTION', 'Temps (en min) :', '#f39c12', (mins) => {
        if (mins && !isNaN(mins) && parseInt(mins) > 0) { 
            const endTime = Date.now() + (parseInt(mins) * 60000);
            localStorage.setItem('urbex_extraction_end', endTime);
            resumeExtractionTimer();
            afficherNotification(`⏱️ Chrono lancé !`, "#f39c12"); 
        } 
    });
} 

function resumeExtractionTimer() {
    const endTime = localStorage.getItem('urbex_extraction_end');
    if (endTime) {
        timeRemaining = Math.floor((parseInt(endTime) - Date.now()) / 1000);
        if (timeRemaining > 0) {
            ...
            localStorage.removeItem('urbex_extraction_end');
        }
    }
}
...
function updateTimer() { 
    ...
    if (timeRemaining === 300) { 
        timerEl.style.background = "#e74c3c"; 
        afficherNotification("⚠️ 5 min restantes !", "#e74c3c"); 
        if ("vibrate" in navigator) navigator.vibrate([500, 200, 500]); 
    } 
}

function cancelExtractionTimer() {
    clearInterval(timerInterval);
    document.getElementById('extraction-timer').classList.add('hidden');
    localStorage.removeItem('urbex_extraction_end');
    afficherNotification("⏱️ Chrono annulé", "#95a5a6");
}
...
// --- 📂 GALERIE INTEL ---
function openIntelGallery() {
    hideAllPanels(); const grid = document.getElementById('gallery-grid'); grid.innerHTML = ''; let hasPhotos = false;
    Object.values(marqueursSurCarte).forEach(markerObj => {
        const data = markerObj.marker.donnees;
        if (data.photo) {
            hasPhotos = true; const item = document.createElement('div'); item.className = 'gallery-item';
            item.innerHTML = `<img src="${data.photo}" onclick="map.flyTo([${data.lat}, ${data.lng}], 18); document.getElementById('gallery-modal').style.display='none';"><div>📍 ${data.nom}</div>`;
            grid.appendChild(item);
        }
    });
    if(!hasPhotos) { grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; color: #7f8c8d; padding: 20px;">Aucune photo trouvée sur la carte.</div>'; }
    document.getElementById('gallery-modal').style.display = 'flex';
}

// --- 🎙️ RECONNAISSANCE VOCALE ---
const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition; const recognition = SpeechRecognition ? new SpeechRecognition() : null; let isDictating = false;
if(recognition) { recognition.lang = 'fr-FR'; recognition.interimResults = false; recognition.maxAlternatives = 1; recognition.onresult = function(event) { document.getElementById('radio-input').value = event.results[0][0].transcript; stopDictationUI(); afficherNotification("🎤 Message capté ! Appuie sur GO.", "#2ecc71"); }; recognition.onerror = function(event) { stopDictationUI(); if(event.error !== 'aborted') afficherNotification("❌ Erreur micro", "#e74c3c"); }; recognition.onend = function() { stopDictationUI(); }; }
...
socket.on('squad_update', (players) => {
    ...
    Object.keys(squadMarkers).forEach(id => { if (!players[id] || id === socket.id) { map.removeLayer(squadMarkers[id]); delete squadMarkers[id]; if(squadTraces[id]) { map.removeLayer(squadTraces[id]); delete squadTraces[id]; } } });
    Object.keys(players).forEach(id => {
        if (id === socket.id) return; const p = players[id];
        ...
        if (p.trace && p.trace.length > 0) { if (!squadTraces[id]) { squadTraces[id] = L.polyline([], {color: '#9b59b6', weight: 3, className: 'teammate-trace'}).addTo(map); } squadTraces[id].setLatLngs(p.trace); }
    });
});
...
function disablePanic() { panicModeActif = false; localStorage.removeItem('panicModeUrbex'); document.querySelector('.top-bar').style.display = 'flex'; document.querySelector('.side-tools').style.display = 'flex'; document.querySelector('.bottom-dock').style.display = 'flex'; document.getElementById('fake-ui').style.display = 'none'; rafraichirFiltresRadar(); map.addLayer(polylineGPS); afficherNotification("👁️ Urbex Restauré", "#8e44ad"); updateTargetLaser(); Object.values(squadMarkers).forEach(m => m.addTo(map)); Object.values(squadTraces).forEach(t => t.addTo(map)); } 
...
map.on('click', function(e) { 
    ...
    if (currentTool === 'patrol') { 
        showSysModal('prompt', '🚨 SÉCURITÉ', 'Quel type ? (Vigile, Chien...)', '#e74c3c', (desc) => {
            if(desc) { const newData = { lat: e.latlng.lat, lng: e.latlng.lng, nom: "🚨 " + desc, date: new Date().toLocaleDateString('fr-FR'), statut: "patrol", personnes: [callsign || "Agent X"], notes: "Sécurité." }; fetch('/api/markers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newData) }); toggleTool('patrol'); }
        });
    } 
});
...
function localiserMoi() {
    if (!callsign) { 
        showSysModal('prompt', '🕵️ IDENTIFICATION', 'Identifiant Agent :', '#3498db', (val) => {
            if(val) { callsign = val; localStorage.setItem('urbex_callsign', callsign); document.getElementById('status-text').innerText = `👋 ${callsign}`; executeLocaliserMoi(); }
        });
    } else { executeLocaliserMoi(); }
}
...
function triggerSOS() { hideAllPanels(); document.getElementById('sos-confirm-modal').style.display = 'flex'; }
...
function creerMarqueurSurCarte(dataMarqueur) { 
    ...
    marker.on('click', async function() { 
        if (currentTool === 'delete') { 
            marker.closePopup(); 
            showSysModal('confirm', '🗑️ SUPPRESSION', `Effacer "${marker.donnees.nom}" ?`, '#e74c3c', (res) => { if (res) fetch(`/api/markers/${marker.donnees.id}`, { method: 'DELETE' }); });
