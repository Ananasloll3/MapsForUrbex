...
let traceGPS = []; let polylineGPS = L.polyline([], {color: '#3498db', dashArray: '8, 8', weight: 4}).addTo(map);
const marqueursSurCarte = {}; const socket = io();

// 📸 NOUVEAU : VARIABLE POUR STOCKER LA PHOTO EN COURS
let currentPhotoBase64 = null;

// --- 📸 LOGIQUE DE COMPRESSION D'IMAGE CÔTÉ CLIENT ---
document.getElementById('point-photo').addEventListener('change', function(e) {
    ...
    reader.onload = function(event) {
        const img = new Image();
        img.onload = function() {
            // Création d'un canvas pour réduire la taille (Max 600px de large)
            ...
            // Compression en JPEG qualité 70%
            ...
            afficherNotification("📸 Photo attachée et compressée !", "#2ecc71");
        }
...
socket.on('ping_tactique', (data) => { if(panicModeActif) return; afficherNotification("📍 Ping d'équipe reçu !", "#2980b9"); const pingIcon = L.divIcon({ className: 'ping-pulse-wrapper', html: '<div class="ping-tactique-ring"></div>', iconSize: [60, 60], iconAnchor: [30, 30] }); const tempPing = L.marker([data.lat, data.lng], {icon: pingIcon, zIndexOffset: 800}).addTo(map); setTimeout(() => { map.removeLayer(tempPing); }, 15000); });
...
socket.on('marker_edited', (pointModifie) => { const markerObj = marqueursSurCarte[pointModifie.id]; if (markerObj) { map.removeLayer(markerObj.marker); if (markerObj.cercle) map.removeLayer(markerObj.cercle); delete marqueursSurCarte[pointModifie.id]; creerMarqueurSurCarte(pointModifie); } chargerMarkers(true); });
socket.on('marker_deleted', (idSupprime) => { const markerObj = marqueursSurCarte[idSupprime]; if (markerObj) { map.removeLayer(markerObj.marker); if (markerObj.cercle) map.removeLayer(markerObj.cercle); delete marqueursSurCarte[idSupprime]; } chargerMarkers(true); });
...
async function chargerMarkers(isBackgroundUpdate = false) { 
    ...
    catch (error) { if (!isBackgroundUpdate) { afficherNotification("⚠️ Réseau KO. Chargement du cache...", "#e74c3c"); const backup = localStorage.getItem('urbex_backup_markers'); if (backup) { const points = JSON.parse(backup); points.forEach(point => creerMarqueurSurCarte(point)); } } }
}
chargerMarkers();
...
function creerMarqueurSurCarte(dataMarqueur) {
    ...
    marker.on('click', async function() {
        if (currentTool === 'delete') { marker.closePopup(); if (confirm(`Supprimer "${marker.donnees.nom}" ?`)) fetch(`/api/markers/${marker.donnees.id}`, { method: 'DELETE' }); } 
            ...
            // 📸 Récupère la photo existante si besoin
            currentPhotoBase64 = marker.donnees.photo || null;
            const preview = document.getElementById('photo-preview');
            if(currentPhotoBase64) { preview.src = currentPhotoBase64; preview.style.display = 'block'; } else { preview.style.display = 'none'; }
            document.getElementById('point-photo').value = ""; // Réinitialise l'input file
...
// --- 📸 Réinitialisation quand on annule ---
document.getElementById('btn-cancel').addEventListener('click', () => { 
    document.getElementById('modal').style.display = 'none'; 
    document.getElementById('point-name').value = ''; document.getElementById('point-notes').value = ''; document.getElementById('point-date').value = ''; document.getElementById('point-status').value = 'accessible'; 
    document.getElementById('people-list').innerHTML = '<input type="text" class="person-input" placeholder="Personne 1">'; 
    
    currentPhotoBase64 = null;
    document.getElementById('point-photo').value = "";
    document.getElementById('photo-preview').style.display = 'none';
    
    tempLatLng = null; markerEnCoursEdition = null; 
});
...
document.getElementById('btn-confirm').addEventListener('click', () => {
    ...
    if (markerEnCoursEdition) { 
        const updatedData = { nom: name, date: date, statut: statut, personnes: peopleNames, notes: notes, photo: currentPhotoBase64 }; 
        ...
        const newData = { lat: tempLatLng.lat, lng: tempLatLng.lng, nom: name, date: date, statut: statut, personnes: peopleNames, notes: notes, photo: currentPhotoBase64 }; 
