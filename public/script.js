...
const marqueursSurCarte = {}; const socket = io();
...
document.getElementById('point-photo').addEventListener('change', function(e) {
    ...
    reader.onload = function(event) {
        ...
        img.onload = function() {
            ...
            const preview = document.getElementById('photo-preview'); preview.src = currentPhotoBase64; preview.style.display = 'block';
            afficherNotification("📸 Photo attachée !", "#2ecc71");
        }
...
// --- 🌑 RAPPORT INTEL (LUNE) ---
function openIntelReport() {
    hideAllPanels();
    const date = new Date();
    // Algorithme simplifié pour calculer l'âge de la lune (0 à 29 jours)
    const lp = 2551443; 
    ...
    let icon = "🌑"; let name = "Nouvelle Lune"; let advice = "Furtivité maximale. Obscurité totale, idéal pour les infiltrations de sites gardés. Lampe infrarouge recommandée.";
    if (days > 1 && days <= 6) { icon = "🌒"; name = "Premier Croissant"; advice = "Légère luminosité. Bonne couverture globale."; }
    else if (days > 6 && days <= 10) { icon = "🌓"; name = "Premier Quartier"; advice = "Luminosité moyenne. Restez dans les ombres portées des bâtiments."; }
    else if (days > 10 && days <= 14) { icon = "🌔"; name = "Lune Gibbeuse"; advice = "Forte visibilité. Évitez les traversées à découvert."; }
    else if (days === 14 || days === 15) { icon = "🌕"; name = "Pleine Lune"; advice = "DANGER : Visibilité maximale. Vous serez visible comme en plein jour à découvert. Déplacements en intérieurs privilégiés."; }
    else if (days > 15 && days <= 19) { icon = "🌖"; name = "Gibbeuse Décroissante"; advice = "Forte visibilité en seconde partie de nuit."; }
    else if (days > 19 && days <= 23) { icon = "🌗"; name = "Dernier Quartier"; advice = "Luminosité moyenne. Lever tardif."; }
    else if (days > 23 && days <= 28) { icon = "🌘"; name = "Dernier Croissant"; advice = "Excellente couverture pour les fins de nuit."; }

    document.getElementById('moon-icon').innerText = icon;
    document.getElementById('moon-phase-name').innerText = name;
    document.getElementById('moon-tactical-advice').innerText = advice;
    document.getElementById('intel-modal').style.display = 'flex';
}

// --- 🎯 LOGIQUE TARGET LOCK ---
window.toggleTargetLock = function(id) {
    if (lockedTargetId === id) {
        // Déverrouiller
        lockedTargetId = null;
        targetLaser.setLatLngs([]);
        document.getElementById('hud-target-row').classList.add('hidden');
        afficherNotification("🔓 Cible désengagée", "#95a5a6");
    } else {
        // Verrouiller
        lockedTargetId = id;
        afficherNotification("🎯 Cible verrouillée ! Suivez l'azimut.", "#e74c3c");
        updateTargetLaser();
    }
    map.closePopup();
};

function updateTargetLaser() {
    if (!lockedTargetId || !userGPSMarker || !marqueursSurCarte[lockedTargetId]) {
        targetLaser.setLatLngs([]);
        document.getElementById('hud-target-row').classList.add('hidden');
        return;
    }
    const targetData = marqueursSurCarte[lockedTargetId].marker.donnees;
    const userLat = userGPSMarker.getLatLng().lat;
    const userLng = userGPSMarker.getLatLng().lng;
    
    // Tracer la ligne
    targetLaser.setLatLngs([[userLat, userLng], [targetData.lat, targetData.lng]]);
    
    // Calcul de l'Azimut (Cap)
    const y = Math.sin((targetData.lng - userLng) * Math.PI / 180) * Math.cos(targetData.lat * Math.PI / 180);
    const x = Math.cos(userLat * Math.PI / 180) * Math.sin(targetData.lat * Math.PI / 180) - Math.sin(userLat * Math.PI / 180) * Math.cos(targetData.lat * Math.PI / 180) * Math.cos((targetData.lng - userLng) * Math.PI / 180);
    const bearing = (Math.atan2(y, x) * 180 / Math.PI + 360) % 360;

    // Mise à jour du HUD
    document.getElementById('hud-target-row').classList.remove('hidden');
    document.getElementById('hud-bearing').innerText = `${Math.round(bearing)}°`;
}
...
function startTimerPrompt() { hideAllPanels(); const mins = prompt("⏱️ Temps avant extraction (en minutes) :\n(Mets 0 pour annuler)"); if (mins && !isNaN(mins) && parseInt(mins) > 0) { timeRemaining = parseInt(mins) * 60; const timerEl = document.getElementById('extraction-timer'); timerEl.classList.remove('hidden'); timerEl.style.background = "#f39c12"; timerEl.style.color = "#fff"; clearInterval(timerInterval); timerInterval = setInterval(updateTimer, 1000); updateTimer(); afficherNotification(`⏱️ Chrono lancé pour ${mins} min`, "#f39c12"); } else if (mins === "0") { clearInterval(timerInterval); document.getElementById('extraction-timer').classList.add('hidden'); afficherNotification("⏱️ Chrono annulé", "#95a5a6"); } }
function updateTimer() { const timerEl = document.getElementById('extraction-timer'); if (timeRemaining <= 0) { clearInterval(timerInterval); timerEl.innerText = "⚠️ GO !"; timerEl.style.background = "#e74c3c"; if ("vibrate" in navigator) navigator.vibrate([1000, 500, 1000, 500, 1000]); afficherNotification("⚠️ TEMPS D'EXTRACTION ÉCOULÉ !", "#e74c3c"); return; } timeRemaining--; const m = Math.floor(timeRemaining / 60).toString().padStart(2, '0'); const s = (timeRemaining % 60).toString().padStart(2, '0'); timerEl.innerText = `⏱️ ${m}:${s}`; if (timeRemaining === 300) { timerEl.style.background = "#e74c3c"; afficherNotification("⚠️ 5 minutes restantes !", "#e74c3c"); if ("vibrate" in navigator) navigator.vibrate([500, 200, 500]); } }

let panicModeActif = false;
function triggerPanic() { panicModeActif = true; localStorage.setItem('panicModeUrbex', 'actif'); hideAllPanels(); document.querySelector('.top-bar').style.display = 'none'; document.querySelector('.side-tools').style.display = 'none'; document.querySelector('.bottom-dock').style.display = 'none'; document.getElementById('fake-ui').style.display = 'block'; document.body.classList.remove('red-mode'); setMapStyle('classique'); Object.values(marqueursSurCarte).forEach(m => { map.removeLayer(m.marker); if (m.cercle) map.removeLayer(m.cercle); }); map.removeLayer(polylineGPS); targetLaser.setLatLngs([]); }
function disablePanic() { panicModeActif = false; localStorage.removeItem('panicModeUrbex'); document.querySelector('.top-bar').style.display = 'flex'; document.querySelector('.side-tools').style.display = 'flex'; document.querySelector('.bottom-dock').style.display = 'flex'; document.getElementById('fake-ui').style.display = 'none'; rafraichirFiltresRadar(); map.addLayer(polylineGPS); afficherNotification("👁️ Mode Urbex Restauré", "#8e44ad"); updateTargetLaser(); }
...
function localiserMoi() {
    ...
    if (gpsLocked) { afficherNotification("🔒 GPS : Suivi & HUD", "#2ecc71"); gpsBtn.classList.add('locked'); hud.classList.remove('hidden'); map.locate({setView: true, maxZoom: 18, watch: true, enableHighAccuracy: true}); } 
...
map.on('locationfound', function(e) {
    ...
    if (gpsLocked) {
        ...
        map.setView(e.latlng, map.getZoom(), { animate: true, duration: 0.5 }); 
        
        // 🧵 MEMORISATION TRACE
        traceGPS.push(e.latlng); polylineGPS.setLatLngs(traceGPS);
        localStorage.setItem('urbex_gps_trace', JSON.stringify(traceGPS)); // Sauvegarde
...
function triggerSOS() { hideAllPanels(); if(confirm("🚨 DÉCLENCHER S.O.S ?")) { if (!navigator.geolocation) return; navigator.geolocation.getCurrentPosition(pos => { socket.emit('sos_alert', { lat: pos.coords.latitude, lng: pos.coords.longitude }); afficherNotification("Signal envoyé !", "#c0392b"); }, err => {}, { enableHighAccuracy: true, timeout: 10000 }); } }
...
async function chargerMarkers(isBackgroundUpdate = false) { 
    try { const response = await fetch('/api/markers'); if (!response.ok) throw new Error("Réseau"); const points = await response.json(); rafraichirCacheLocal(points); if (!isBackgroundUpdate) { points.forEach(point => creerMarqueurSurCarte(point)); } } 
    catch (error) { if (!isBackgroundUpdate) { afficherNotification("⚠️ Mode Survie : Cache chargé", "#e74c3c"); const backup = localStorage.getItem('urbex_backup_markers'); if (backup) { const points = JSON.parse(backup); points.forEach(point => creerMarqueurSurCarte(point)); } } }
}
chargerMarkers();
...
function toggleTool(tool) {
    ...
    document.getElementById('btn-add').classList.toggle('active-add', currentTool === 'add'); document.getElementById('btn-edit').classList.toggle('active-edit', currentTool === 'edit'); document.getElementById('btn-del').classList.toggle('active-del', currentTool === 'delete'); document.getElementById('btn-measure').classList.toggle('active-measure', currentTool === 'measure'); document.getElementById('btn-patrol').classList.toggle('active-patrol', currentTool === 'patrol');
...
function creerIcone(statut) {
    let bgClass = 'bg-accessible'; if (statut === 'garde') bgClass = 'bg-garde'; if (statut === 'dangereux') bgClass = 'bg-dangereux'; if (statut === 'detruit') bgClass = 'bg-detruit'; if (statut === 'patrol') bgClass = 'bg-patrol'; if (statut === 'extraction') bgClass = 'bg-extraction';
...
function genererPopupHTML(data) {
    ...
    if (data.statut === 'garde') { badgeText = "Gardé / Caméras"; badgeClass = "badge-garde"; } if (data.statut === 'dangereux') { badgeText = "Dangereux / Ruine"; badgeClass = "badge-dangereux"; } if (data.statut === 'detruit') { badgeText = "Détruit / Muré"; badgeClass = "badge-detruit"; } if (data.statut === 'patrol') { badgeText = "🚨 Sécurité Signalée"; badgeClass = "badge-dangereux"; } if (data.statut === 'extraction') { badgeText = "🚗 Point d'Extraction"; badgeClass = "badge-extraction"; }
    ...
    // 🎯 NOUVEAU : BOUTON TARGET LOCK
    const isLocked = lockedTargetId === data.id;
    const lockBtnClass = isLocked ? 'btn-target-lock locked-active' : 'btn-target-lock';
    const lockBtnText = isLocked ? '🔓 Désengager la cible' : '🎯 Verrouiller Cible';
    const targetLockBtn = `<button class="${lockBtnClass}" onclick="window.toggleTargetLock('${data.id}')">${lockBtnText}</button>`;
...
function creerMarqueurSurCarte(dataMarqueur) {
    ...
    if (dataMarqueur.statut === 'garde' || dataMarqueur.statut === 'dangereux' || dataMarqueur.statut === 'patrol') {
        const estGarde = dataMarqueur.statut === 'garde'; dangerCircle = L.circle([dataMarqueur.lat, dataMarqueur.lng], { color: estGarde ? '#f39c12' : '#e74c3c', fillColor: estGarde ? '#f39c12' : '#e74c3c', fillOpacity: 0.15, radius: estGarde ? 150 : 100 });
    ...
    marker.on('click', async function() {
        if (currentTool === 'delete') { marker.closePopup(); if (confirm(`Supprimer "${marker.donnees.nom}" ?`)) fetch(`/api/markers/${marker.donnees.id}`, { method: 'DELETE' }); } 
            ...
            currentPhotoBase64 = marker.donnees.photo || null; const preview = document.getElementById('photo-preview'); if(currentPhotoBase64) { preview.src = currentPhotoBase64; preview.style.display = 'block'; } else { preview.style.display = 'none'; } document.getElementById('point-photo').value = "";
...
document.getElementById('btn-cancel').addEventListener('click', () => { document.getElementById('modal').style.display = 'none'; document.getElementById('point-name').value = ''; document.getElementById('point-notes').value = ''; document.getElementById('point-date').value = ''; document.getElementById('point-status').value = 'accessible'; document.getElementById('people-list').innerHTML = '<input type="text" class="person-input" placeholder="Personne 1">'; currentPhotoBase64 = null; document.getElementById('point-photo').value = ""; document.getElementById('photo-preview').style.display = 'none'; tempLatLng = null; markerEnCoursEdition = null; });
document.getElementById('btn-confirm').addEventListener('click', () => {
    const name = document.getElementById('point-name').value.trim() || "Spot"; const date = document.getElementById('point-date').value || "Inconnue"; const statut = document.getElementById('point-status').value; const notes = document.getElementById('point-notes').value.trim();
    ...
    if (markerEnCoursEdition) { const updatedData = { nom: name, date: date, statut: statut, personnes: peopleNames, notes: notes, photo: currentPhotoBase64 }; fetch(`/api/markers/${markerEnCoursEdition.donnees.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updatedData) }); } 
    else { const newData = { lat: tempLatLng.lat, lng: tempLatLng.lng, nom: name, date: date, statut: statut, personnes: peopleNames, notes: notes, photo: currentPhotoBase64 }; fetch('/api/markers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newData) }); }
